...
func (state *State) Reset(rStr, sStr string) error {
	...
	rows, err := state.db.Query(
		...
		util.ArrayToString(state.initialRNGVector[:]),
		util.ArrayToString(state.resetToken[:]),
		rStr, sStr)
...
func (state *State) AdvanceState() uint32 {
	...
	result := util.RotL(state.currentRNGVector[0]+state.currentRNGVector[3], 7) + state.currentRNGVector[0]
	...
	state.currentRNGVector[3] = util.RotL(state.currentRNGVector[3], 11)
