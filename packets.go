...
type PacketWrapper struct {
	...
	PacketData interface{} `json:"data"`
}

func ParsePackets[T EssentialsPacket | FullPacket](body []byte) (packets []PacketWrapper, err error) {
	if len(body) < 128+2 {
	...
	if !ecdsa.Verify(&publicKey, jsonHash[:], r, s) {
	...
	for k, v := range packets {
		...
		var packet T
		err = mapstructure.Decode(v.PacketData, &packet)
		if err != nil {
			err = errors.New(fmt.Sprintf("packet at index %d was not of the correct type", k))
			return
		}

		packets[k].PacketData = packet
	}
